...
const register = async (req: Request, res: Response) => {
  ...
  const { hos_id, group_id, user_name, user_password, display_name, user_status, user_level, create_by }: UserRegister = req.body;
  const saltRounds = 10;
  bcrypt.hash(user_password, saltRounds, async (err, hash) => {
    ...
    await prisma.users.create({
      ...
      fs.unlinkSync(path.join('public/images', String(req.file?.filename)))
